	...
	*/
	...
	for (j = 0; j < mf; j++) {
		for (i = 0;i < mf; i++) {
			...
			ap = (int)floor(alpha0);
			//DbgMsg(" ap: " << ap);
			if ((ap + qr - 1) < 0 || (ap - qr) > a2) //check boundaries, assuming only positive alpha range
			{
				continue;
			}
			for (k = 0; k < KN; k++) {
...
int AFAM(float* x, float &da, int a1, int a2, int L, int Nw, int opt, float* SCo)
{
	/*
